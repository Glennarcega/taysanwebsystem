...
if (isset($_GET['transaction_id'])) {
    ...
    if (is_numeric($transaction_id)) {
        ...
        $query = "UPDATE transaction SET status = 'Reserved' WHERE transaction_id = ?";
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->affected_rows > 0) {
                ...
                echo "<script>alert('Transaction reserved successfully!'); window.location.href = '../admin/reserve1.php';</script>";
            } else {
                // If no rows were affected, the transaction may not exist or is already reserved
                echo "<script>alert('Error updating status. Please try again.'); window.location.href = '/admin/go_checkin.php';</script>";
            }

            $stmt->close();
        } else {
            // Handle any errors with the query preparation
            echo "<script>alert('Failed to prepare the query.'); window.location.href = '/admin/go_checkin.php';</script>";
        }
    } else {
        // If transaction_id is not a valid number
        echo "<script>alert('Invalid transaction ID.'); window.location.href = '/admin/go_checkin.php';</script>";
    }
} else {
    // If the transaction_id is not set in the URL
    echo "<script>alert('Transaction ID not found.'); window.location.href = '/admin/go_checkin.php';</script>";
}

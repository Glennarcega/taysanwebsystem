...
if (isset($_GET['transaction_id'])) {
    ...
    if (is_numeric($transaction_id)) {
        ...
        $query = "UPDATE transactionresort SET status = 'Check In', checkin_time = ? WHERE transaction_id = ?";
        ...
        if ($stmt = $conn->prepare($query)) {
            ...
            if ($stmt->affected_rows > 0) {
                // If the update was successful, redirect to the checkin_booking_resort.php page
                echo "<script>alert('Transaction Check-in successful!'); window.location.href = '../admin/checkin_booking_resort.php';</script>";
            } else {
